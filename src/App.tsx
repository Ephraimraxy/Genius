...
export default function App() {
  ...
  const renderContent = () => {
    ...
    switch (activeTab) {
      ...
      case 'quick_publish': return <QuickPublishPage activePaperId={activePaperId} setActivePaperId={setActivePaperId} token={token} addToast={addToast} isAdmin={isAdmin} completeLabel={isAdmin ? 'Go to Global Reviews' : 'Go to Publication Records'} onComplete={() => { setActiveTab(isAdmin ? 'globalReviews' : 'records'); setActivePaperId(null); }} />;
          ...
          <div className="flex items-center gap-2 sm:gap-3 lg:gap-6">
            ...
            {activePaperId && !['dashboard', 'profile', 'users', 'reviewQueue', 'settings'].includes(activeTab) && (
              <motion.div
                ...
                className="hidden xl:flex items-center gap-2 px-4 py-2 bg-indigo-50 border border-indigo-100 rounded-2xl"
              >
