    ...
    <div className="fixed bottom-28 sm:bottom-8 right-4 sm:right-8 z-[100]">
      <AnimatePresence>
        {isOpen && (
          ...
          >
            {/* Header */}
            <div className={`p-6 ${isAdmin ? 'bg-gradient-to-br from-slate-900 to-[#800000]' : 'bg-gradient-to-br from-indigo-900 to-indigo-600'} text-white flex items-center justify-between shrink-0`}>
            ...
            {isAdmin && !activeThread ? (
              ...
              <>
                <div className="flex-1 overflow-y-auto p-6 space-y-4 custom-scrollbar bg-slate-50/50">
                  {messages.length === 0 && (
                    <div className="h-full flex flex-col items-center justify-center text-center p-8">
                      <div className={`w-16 h-16 rounded-full flex items-center justify-center mb-4 ${isAdmin ? 'bg-slate-900/5 text-slate-900' : 'bg-indigo-900/5 text-indigo-900'}`}>
                    ...
                    return (
                      <div key={i} className={`flex ${isOwnMessage ? 'justify-end' : 'justify-start'}`}>
                        <div className={`max-w-[80%] p-4 rounded-2xl text-xs font-medium leading-relaxed shadow-sm ${
                          isOwnMessage 
                            ? (isAdmin ? 'bg-slate-900 text-white rounded-tr-none' : 'bg-indigo-600 text-white rounded-tr-none')
                ...
                <form onSubmit={handleSend} className="p-4 border-t border-slate-100 bg-white shrink-0">
                  <div className={`flex items-center gap-2 bg-slate-50 rounded-2xl p-2 border border-slate-100 focus-within:ring-2 transition-all ${isAdmin ? 'focus-within:ring-slate-900/10' : 'focus-within:ring-indigo-600/10'}`}>
                    <input
                      type="text"
                      value={input}
                    ...
                    <button 
                      ...
                      className={`p-3 text-white rounded-xl shadow-lg transition-all disabled:opacity-50 disabled:scale-100 ${isAdmin ? 'bg-slate-900 hover:scale-105' : 'bg-indigo-600 hover:scale-105 shadow-indigo-600/20'}`}
                    >
      ...
      <motion.button
        ...
        className={`w-14 h-14 rounded-full shadow-2xl flex items-center justify-center text-white transition-all transform duration-300 ${
          isOpen ? 'bg-slate-800 rotate-90' : isAdmin ? 'bg-slate-900' : 'bg-indigo-600 shadow-indigo-600/30'
        }`}
      >
